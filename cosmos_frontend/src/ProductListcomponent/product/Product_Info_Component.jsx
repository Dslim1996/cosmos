...
import test from "../../ProductListcomponent/product/test";
...
function ProductInfoComponent(props){
    ...
    const setcolor = Array.from(new Set(product && product.map(option => option.product_color ))); //상품 컬러 중복 제거
    const setimgs = Array.from(new Set(product && product.map(option => option.imgs))); //상품 이미지 중복 제거 (수정하세요)
    const history = useHistory(); 
    let shopcart;
    let prop = [];
    let Esize = []; //화면에 보여줄 사이즈 변수
